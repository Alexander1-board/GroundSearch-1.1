...
const App: React.FC = () => {
  ...
  const handleGeneratePlan = async (jobId: string) => {
    ...
    try {
      ...
      logTrace({
        ...
        action: 'INFO',
        status: 'info',
        summary: 'Auto-starting execution...',
      });
  ...
  const handleManualIngest = async (job: ResearchJob, step: ExecutionStep, content: string) => {
    ...
    try {
          ...
          steps: job.plan!.steps.map((s) =>
            s.id === step.id
              ? {
                  ...
                  duration: (s.duration || 0) + manualIngestDuration,
                }
              : s,
          ),
        },
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error during manual ingest.';
      const manualIngestDuration = Date.now() - manualIngestStartTime;
      logTrace({
        step_id: step.id,
        action: 'ERROR',
  ...
  if (isLoading) {
    return (
      <div className="w-screen h-screen flex items-center justify-center bg-light-bg dark:bg-dark-bg">
        <div className="text-xl text-light-text/60 dark:text-dark-text/60">
          Loading GroundSearch...
        </div>
      </div>
    );
  }
      ...
      <div className="flex-grow flex flex-col relative">
        ...
        {config.isWolframEnabled && !config.wolframAppId && (
          <div className="absolute bottom-4 right-4 bg-warning text-black p-3 rounded-lg shadow-lg text-sm z-20">
            <strong>Configuration Warning:</strong> Wolfram|Alpha agent is enabled, but the App ID
            is missing. This agent will be skipped.
          </div>
        )}
      </div>
      <ConfigModal
