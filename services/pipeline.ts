...
import { ResearchJob, ExecutionStep, TraceEvent, RecordLite, Evidence, InsightPackResult, FacetResult } from '../types';
...
const updateStepInJob = (job: ResearchJob, stepId: string, updates: Partial<ExecutionStep>): ResearchJob => {
...
export async function executePlan(initialJob: ResearchJob, callbacks: PipelineCallbacks, resume: boolean = false, stepIdToResume?: string) {
    ...
    if (resume) {
        logHelper({ action: 'INFO', summary: `Resuming plan execution${stepIdToResume ? ` from step ${stepIdToResume}`: ''}.` });
    ...
    let startIndex = resume ? stepsToRun.findIndex(s => s.id === stepIdToResume || ['Pending', 'Failed', 'CORS_BLOCKED'].includes(s.status)) : 0;
    ...
    for (let i = startIndex; i < stepsToRun.length; i++) {
        ...
        logHelper({ 
            ...
            model: ['SEARCH', 'INGEST', 'RESEARCH_FACET'].includes(step.action) ? currentJob.models.tool : currentJob.models.reasoning,
            input_snapshot: { params: step.params, specialist_instructions: step.specialist_instructions }
        });
        updateJob(currentJob);
        ...
        try {
            ...
            switch (step.action) {
                case 'RESEARCH_FACET': {
                    ...
                    setJobStatus('Running');
                    const { resultText, sources } = await GeminiService.runFacetResearchAgent(currentJob.models.reasoning, step.params.facet_name, currentJob.brief);
                ...
                case 'SYNTHESIZE_RESEARCH': {
                     ...
                     const deepResearchResult = await GeminiService.runResearchSynthesisAgent(currentJob.models.reasoning, JSON.stringify(accumulatedFacetResults), uniqueSources);
                     
                     const evidenceFromDeepResearch: Evidence[] = (deepResearchResult.sources || []).map((record: RecordLite) => {
                     ...
                     currentJob = { ...currentJob, deepResearchResult, evidence: evidenceFromDeepResearch, sources: [] }; // Reset sources after synthesis
                     break;
                }
                case 'SEARCH': {
                    ...
                    if (currentJob.config.isWolframEnabled && currentJob.config.wolframAppId) {
                         searchPromises.push(Tools.wolfram_query(step.params.query, currentJob.config.wolframAppId));
                    }
                    ...
                    settledResults.forEach((res, index) => {
                        ...
                        if(res.status === 'fulfilled') {
                            logHelper({ step_id: step.id, action: 'INFO', status: 'info', summary: `${sourceName} found ${res.value.length} sources.` });
                            allSources.push(...res.value);
                        } else {
                             logHelper({ step_id: step.id, action: 'ERROR', status: 'failed', summary: `${sourceName} failed.`, error: res.reason.message });
                        }
                    });

                    result = allSources;
                    
                    const newSources = [...(currentJob.sources || []), ...(Array.isArray(result) ? result : [])];
                ...
                case 'INGEST':
                    try {
                        ...
                        if (e instanceof Error && e.message === 'CORS_BLOCKED') {
                            ...
                            currentJob = updateStepInJob(currentJob, step.id, { status: 'CORS_BLOCKED', duration: duration_ms });
                            setJobStatus('Running');
                            updateJob(currentJob);
                ...
                case 'SCREEN': {
                    ...
                    if (sourcesToScreen.length === 0) {
                        logHelper({ step_id: step.id, parent_step_id: step.action, action: 'INFO', status: 'info', summary: 'No sources found to screen. Skipping step.' });
                    ...
                    for (let i = 0; i < sourcesToScreen.length; i += BATCH_SIZE) {
                        ...
                        totalDropped += batchResult.dropped_count || (batch.length - (batchResult.kept?.length || 0));
                    ...
                    if (scoredEvidence.length === 0) {
                        ...
                        logHelper({ 
                            step_id: step.id, parent_step_id: step.action, action: step.action, agent: step.agent, status: 'failed', 
                            summary: message, 
                            recommendation: 'Try refining the research scope or search queries in the plan.',
                            error: message, output_snapshot: screeningResult
                        });
                        throw new Error(message);
                    }
                    
                    result = { kept: scoredEvidence, dropped_count: screeningResult.dropped_count };
                    logHelper({ step_id: step.id, parent_step_id: step.action, action: 'INFO', status: 'info', summary: `Screening complete. Kept and scored ${result.kept.length} sources, dropped ${result.dropped_count}.` });
                ...
                case 'COMPARE':
                    ...
                    const rawInsightPack = await GeminiService.runInsightPackAgent(currentJob.models.reasoning, currentJob);
            ...
            const duration_ms = Date.now() - startTime;
            currentJob = updateStepInJob(currentJob, step.id, { status: 'Completed', result, duration: duration_ms });
            updateJob(currentJob);
logHelper({ 
                step_id: step.id, parent_step_id: step.action, action: step.action, agent: step.agent, status: 'success', 
                summary: `Step completed successfully.`, 
                end_ts: Date.now(), duration_ms, output_snapshot: result 
            });
            ...
            logHelper({ 
                ...
                end_ts: Date.now(), duration_ms, error: errorMessage,
                recommendation: 'Please check the error and retry the step.'
            });
            currentJob = updateStepInJob(currentJob, step.id, { status: 'Failed', result: errorMessage, duration: duration_ms });
            updateJob({ ...currentJob, status: 'Error', lastError: `Step '${step.action}' failed: ${errorMessage}` });
            return;
        }
    }

    setJobStatus('Complete');
    logHelper({ step_id: 'system', action: 'INFO', status: 'success', summary: 'Plan finished successfully.' });
}