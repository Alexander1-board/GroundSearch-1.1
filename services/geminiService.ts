...
async function retry<T>(op: () => Promise<T>, tries: number = 6): Promise<T> {
  ...
  for (let i = 0; i < tries; i++) {
    try {
      ...
      lastErr = e;
      if (i < tries - 1) {
        const isRateLimitError = e.message && (e.message.includes('429') || e.message.includes('RESOURCE_EXHAUSTED'));
        if (isRateLimitError) {
          // Much more aggressive backoff. i=0->5s, i=1->10s, i=2->20s, i=3->40s, i=4->80s
          const backoffTime = (5000 * Math.pow(2, i)) + (Math.random() * 1000);
          console.warn(`Attempt ${i + 1}/${tries - 1} failed due to rate limit. Retrying in ${backoffTime.toFixed(0)}ms...`, e.message);
    ...
    try {
        ...
        if (isJson) {
            ...
            try {
                ...
                throw new Error(`Malformed JSON response: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
            }
        }
        return text as T;
    } catch (error) {
...
const executionStepSchema = {
    ...
    properties: {
        ...
        action: { type: Type.STRING, enum: ['SEARCH', 'SCREEN', 'COMPARE', 'ANSWER', 'INGEST', 'RESEARCH_FACET', 'SYNTHESIZE_RESEARCH'] },
        params: {
            type: Type.OBJECT,
            properties: {
        ...
        expects: { type: Type.STRING, enum: ['RecordLite[]', 'Evidence[]', 'Report', 'FinalAnswer', 'FacetResult', 'DeepResearchResult'] },
...
const evidenceSchema = {
    ...
    required: [ 'id', 'title', 'url', 'source_id', 'matches_questions', 'within_timeframe', 'has_comparators', 'has_quant_outcomes' ]
};
...
const sourceAnalysisSchema = {
    ...
    properties: {
        source_id: { type: Type.STRING },
        role: { type: Type.STRING, enum: ['primary', 'official', 'peer_reviewed', 'reputable_media', 'blog', 'forum'] },
...
const finalAnswererResultSchema = {
    ...
    required: ["answer", "markdown_body", "theme_confidences", "next_steps", "high_value_sources", "uncertainties", "rationale_summary"]
};
...
const deepResearchResultSchema = {
    ...
    properties: {
        facet_results: {
            ...
            required: ["governance", "ownership", "control", "decentralization_claims", "technical_architecture"],
            additionalProperties: true,
        },
...
export const runConversationAgent = (model: string, history: ResearchJob['chatHistory'], currentBrief: ResearchBrief) => {
    ...
    return retry(() => withTimeout(() => callModelAPI<{ reply: string; outline: ResearchBrief }>(model, prompt, true, schema), TIMEOUTS.conversation));
};
...
export const runOrchestrationAgent = (model: string, brief: ResearchBrief) => {
    ...
    return retry(() => withTimeout(() => callModelAPI<ExecutionPlan>(model, prompt, true, executionPlanSchema), TIMEOUTS.plan));
};

export const runFacetResearchAgent = async (model: string, facetName: string, brief: ResearchBrief): Promise<{ resultText: string, sources: RecordLite[] }> => {
...
export const runResearchSynthesisAgent = (model: string, facetSummaries: string, sources: RecordLite[]) => {
    const promptText = AgentPrompts.getResearchSynthesisAgentPrompt(facetSummaries, sources);
    const prompt: Content[] = [{ role: 'user', parts: [{ text: promptText }] }];
    
    return retry(() => withTimeout(() => callModelAPI<DeepResearchResult>(
        model, 
        prompt, 
        true, 
...
export const runScreeningAgent = (model: string, records: RecordLite[], brief: ResearchBrief) => {
    ...
    return retry(() => withTimeout(() => callModelAPI<{ kept: Evidence[], dropped_count: number }>(
        model, 
        prompt, 
        true, 
...
export const runInsightPackAgent = (model: string, job: ResearchJob): Promise<InsightPackResult> => {
    ...
    return retry(() => withTimeout(() => callModelAPI<InsightPackResult>(
        model, 
        prompt, 
        true, 
        insightPackSchema,
...
export const runAnswererAgent = (model: string, job: ResearchJob): Promise<FinalAnswererResult> => {
    ...
    return retry(() => withTimeout(() => callModelAPI<FinalAnswererResult>(
        model, 
        prompt, 
        true, 
...
export const runFollowUpChat = (model: string, job: ResearchJob, history: ChatMessage[], newQuestion: string): Promise<string> => {
    ...
    return retry(() => withTimeout(() => callModelAPI<string>(
        model, 
        prompt, 
        false, // isJson = false
        undefined,
