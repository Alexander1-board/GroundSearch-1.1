...
import PrePromptPanel from './PrePromptPanel';

// --- Tooltip Component ---
const Tooltip: React.FC<{ children: React.ReactNode; content: React.ReactNode | string }> = ({
  children,
  content,
}) => {
  return (
...
const ScopeTab: React.FC<{
  ...
  const handleSend = async () => {
    if (!input.trim() || isThinking || apiKeyMissing) return;
    ...
    try {
      ...
      const result = await GeminiService.runConversationAgent(
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
      ...
      <div className="flex flex-col bg-light-surface dark:bg-dark-surface rounded-lg p-4 h-full">
        <div className="flex-grow overflow-y-auto pr-2 space-y-4 mb-4">
          {job.chatHistory.map((msg, index) => (
            ...
            >
              <div
                className={`max-w-lg p-3 rounded-lg break-words whitespace-pre-wrap ${msg.role === 'user' ? 'bg-secondary text-dark-text' : 'bg-light-bg dark:bg-dark-bg border border-secondary/10 dark:border-primary/10'} break-words whitespace-pre-wrap `}
              >
        ...
        <div className="flex gap-2 border-t border-secondary/10 dark:border-primary/10 pt-4">
          ...
          <button
            onClick={handleSend}
            disabled={isThinking || apiKeyMissing}
            className="bg-primary hover:bg-primary/90 text-secondary font-bold py-2 px-4 rounded-md disabled:bg-light-text/20 dark:disabled:bg-dark-text/20 self-end"
          >
            Send
          </button>
        </div>
      ...
      <div className="flex flex-col bg-light-surface dark:bg-dark-surface rounded-lg p-4 h-full overflow-y-auto">
        ...
        <div className="space-y-4 text-sm">
          <BriefInput
            label="Objective"
            value={job.brief.objective}
            onChange={(v) => handleBriefChange('objective', v)}
          />
          <BriefTextarea
            label="Key Questions"
            value={job.brief.key_questions.join('\n')}
            onChange={(v) => handleBriefChange('key_questions', v.split('\n'))}
          />
          <BriefInput
            label="Deliverable"
            value={job.brief.deliverable}
            onChange={(v) => handleBriefChange('deliverable', v)}
          />
          <BriefTextarea
            label="Success Criteria"
            value={job.brief.success_criteria.join('\n')}
          ...
          <BriefInput
            label="From Year"
            type="number"
            value={job.brief.scope.timeframe.from}
            onChange={(v) =>
              handleScopeChange('timeframe', { ...job.brief!.scope.timeframe, from: Number(v) })
            }
          />
          <BriefInput
            label="To Year"
            type="number"
            value={job.brief.scope.timeframe.to}
            onChange={(v) =>
              handleScopeChange('timeframe', { ...job.brief!.scope.timeframe, to: Number(v) })
            }
          />
          <BriefTextarea
            label="Domains/URLs"
            value={job.brief.scope.domains.join('\n')}
    ...
    <div className="text-xs">
      ...
      <div className="space-y-2 mt-3">
        {(Object.entries(result.facet_results) as [string, FacetResult][]).map(([name, facet]) => (
          <div
            key={name}
            className="bg-light-bg dark:bg-dark-bg rounded-md border border-secondary/10 dark:border-primary/10"
          >
            <button
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-semibold ${coverageColors[facet.coverage]}`}
                >
                  {facet.coverage}
                </span>
                <ChevronDownIcon
                  className={`w-4 h-4 transition-transform ${openFacets[name] ? 'rotate-180' : ''}`}
                />
              </div>
            </button>
            {openFacets[name] && (
              <div className="p-3 border-t border-secondary/10 dark:border-primary/10 space-y-3">
                ...
                {facet.claims.map((claim, i) => (
                  <div key={i} className="p-2 rounded-md bg-light-surface dark:bg-dark-surface">
                    <p>
                      <span className="font-semibold">Claim:</span> {claim.summary}
                    </p>
                    <p>
                      <span className="font-semibold">Confidence:</span>{' '}
                      <span className="capitalize">{claim.confidence}</span>
                    </p>
                    <p>
                      <span className="font-semibold">Sources:</span> {claim.source_ids.join(', ')}
                    </p>
                  </div>
                ))}
                <div>
                  ...
                  <ul className="list-disc list-inside pl-2 font-mono text-light-text/70 dark:text-dark-text/70 text-[10px]">
                    {facet.tried.queries.map((q, i) => (
                      <li key={i}>{q}</li>
                    ))}
                    {facet.tried.auxiliary.map((a, i) => (
                      <li key={i}>Checked: {a}</li>
                    ))}
                  </ul>
                </div>
              </div>
...
const ResultsTab: React.FC<{
  ...
  apiKeyMissing: boolean;
}> = ({ job, updateJob, handleFollowUpChat, apiKeyMissing }) => {
  ...
  const handleSendFollowUp = async () => {
    if (!followUpInput.trim() || isChatting || apiKeyMissing) return;
  ...
  if (job.status !== 'Complete' || !job.answererResult) {
    return (
      <div className="text-center p-8">
        <p>Research is not yet complete.</p>
      </div>
    );
  }

  const { answer, markdown_body, theme_confidences, high_value_sources, next_steps } =
    job.answererResult;
    ...
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
        <div role="tablist" className="tabs tabs-boxed bg-light-surface dark:bg-dark-surface p-1">
          <a
            role="tab"
            className={`tab text-sm ${view === 'report' ? 'tab-active !bg-secondary !text-dark-text' : ''}`}
          ...
          <a
            role="tab"
            className={`tab text-sm ${view === 'matrix' ? 'tab-active !bg-secondary !text-dark-text' : ''}`}
        ...
        {view === 'report' && (
          <div className="flex gap-2">
            <button
              ...
              className="bg-light-surface dark:bg-dark-surface border border-secondary/20 dark:border-primary/20 hover:bg-primary/10 dark:hover:bg-primary/10 font-bold py-2 px-4 rounded-md text-sm"
            >
              Copy Answer
            </button>
            <button
              onClick={downloadMarkdown}
              className="bg-secondary hover:bg-secondary/90 text-dark-text font-bold py-2 px-4 rounded-md text-sm"
            >
              Download .md
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            <div className="lg:col-span-2 bg-light-surface dark:bg-dark-surface rounded-lg p-6 h-full overflow-y-auto break-words flex flex-col">
              <div>
                ...
                <p className="text-lg font-bold mb-4 border-b border-secondary/10 dark:border-primary/10 pb-4">
                  {answer}
                </p>
              ...
              <div className="flex-grow flex flex-col mt-6 pt-6 border-t border-secondary/10 dark:border-primary/10 min-h-[200px]">
                <div className="flex-grow overflow-y-auto pr-2 space-y-4 mb-4">
                  {job.followUpHistory.map((msg, index) => (
                    ...
                    >
                      <div
                        className={`max-w-lg p-3 rounded-lg break-words whitespace-pre-wrap ${msg.role === 'user' ? 'bg-secondary text-dark-text' : 'bg-light-bg dark:bg-dark-bg border border-secondary/10 dark:border-primary/10'}`}
                      >
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={isChatting || !followUpInput.trim() || apiKeyMissing}
                    className="bg-primary hover:bg-primary/90 text-secondary font-bold py-2 px-4 rounded-md disabled:bg-light-text/20 dark:disabled:bg-dark-text/20 self-end"
                  >
                    Send
                  </button>
            ...
            <div className="lg:col-span-1 bg-light-surface dark:bg-dark-surface rounded-lg p-4 h-full overflow-y-auto flex flex-col gap-4">
              <div>
                ...
                <div className="space-y-2 text-xs">
                  {theme_confidences?.map((tc, i) => (
                    <div
                      key={i}
                      className="bg-light-bg dark:bg-dark-bg border-l-4 p-2 rounded-r-md"
                      style={{
                        borderColor:
                          tc.confidence === 'high'
                            ? 'var(--tw-color-success)'
                            : tc.confidence === 'medium'
                              ? 'var(--tw-color-warning)'
                              : 'var(--tw-color-error)',
                      }}
                    >
              ...
              {job.deepResearchResult && (
                <div>
                  <button
                    onClick={() => setIsDeepResearchVisible(!isDeepResearchVisible)}
                    className="w-full flex justify-between items-center font-bold"
                  >
                    <span>Deep Research Log</span>
                    <ChevronDownIcon
                      className={`w-4 h-4 transition-transform ${isDeepResearchVisible ? 'rotate-180' : ''}`}
                    />
                  </button>
              ...
              <div>
                <button
                  onClick={() => setIsHvSourcesVisible(!isHvSourcesVisible)}
                  className="w-full flex justify-between items-center font-bold"
                >
                  <span>High-Value Source Analysis</span>
                  <ChevronDownIcon
                    className={`w-4 h-4 transition-transform ${isHvSourcesVisible ? 'rotate-180' : ''}`}
                  />
                </button>
                {isHvSourcesVisible && (
                  <div className="space-y-2 text-xs mt-2">
                    {high_value_sources?.map((hvs, i) => (
                      <div
                        key={i}
                        className="bg-light-bg dark:bg-dark-bg p-2 rounded-md border border-secondary/10 dark:border-primary/10"
                      >
                        <p className="font-bold">
                          {hvs.id}:{' '}
                          {job.evidence?.find((e) => e.id === hvs.id)?.title || 'Unknown Source'}
                        </p>
                        <p className="text-light-text/80 dark:text-dark-text/80 italic">
                          Why: {hvs.why}
                        </p>
                      </div>
                    ))}
              ...
              <div>
                ...
                <div className="space-y-3 text-xs">
                  {job.evidence?.map((e, i) => (
                    <div
                      key={e.id}
                      className="bg-light-bg dark:bg-dark-bg border border-secondary/10 dark:border-primary/10 p-2 rounded-md"
                    >
...
const OrchestrationStepCard: React.FC<{
  ...
  const statusIcons: Record<string, React.ReactNode> = {
    Pending: (
      <div className="w-3 h-3 rounded-full bg-light-text/40 dark:bg-dark-text/40 animate-pulse"></div>
    ),
  ...
  const tooltipContent = (
    <div className="p-1 text-left">
      <p>
        <strong>Agent:</strong> {latestEvent.agent || 'N/A'}
      </p>
      <p>
        <strong>Model:</strong> {latestEvent.model || 'N/A'}
      </p>
      <p>
        <strong>Status:</strong> {step.status}
      </p>
      <p className="mt-1 pt-1 border-t border-dark-text/20">
        <em>{latestEvent.summary}</em>
      </p>
    </div>
  );
  ...
  return (
    <div
      onClick={onSelect}
      className={`bg-light-bg dark:bg-dark-bg p-3 rounded-md border-l-4 ${isSelected ? 'border-secondary' : 'border-primary/50'} cursor-pointer hover:border-secondary transition-all`}
    >
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3 min-w-0">
          <Tooltip content={tooltipContent}>{statusIcons[step.status]}</Tooltip>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-light-text/70 dark:text-dark-text/70 truncate">
              {latestEvent.summary || step.specialist_instructions || '...'}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {latestEvent.fallback_used && (
            <span className="text-xs bg-yellow-400/20 text-yellow-600 dark:text-yellow-300 px-2 py-0.5 rounded-full">
              Fallback
            </span>
          )}
          {step.duration && (
          ...
          {stepEvents.length > 0 && (
            ...
            >
              <ChevronDownIcon
                className={`w-4 h-4 transition-transform ${isLogVisible ? 'rotate-180' : ''}`}
              />
            </button>
          )}
        </div>
  ...
  if (typeof data === 'object') {
    content = (
      <div className="relative">
        <pre className="text-[11px] bg-black/5 dark:bg-black/20 p-2 rounded mt-1 max-h-60 overflow-y-auto">
          {JSON.stringify(data, null, 2)}
        </pre>
        <button
          onClick={copyToClipboard}
          className="absolute top-2 right-2 p-1 bg-light-surface dark:bg-dark-surface rounded-md hover:bg-primary/20"
        >
          {copied ? (
...
const DetailPanel: React.FC<{
  ...
  if (!step) {
    return (
      <div className="text-center p-4 text-sm text-light-text/60 dark:text-dark-text/60">
    ...
    <div className="p-4 h-full flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto">
        <div className="text-sm space-y-1">
          ...
          <p>
            <strong>Agent:</strong> {event?.agent || step.agent}
          </p>
          <p>
            <strong>Model:</strong> {event?.model || 'N/A'}
          </p>
          <p>
            <strong>Duration:</strong> {step.duration ? `${step.duration / 1000}s` : 'N/A'}
          </p>
        </div>
        {event?.error && !isCorsBlocked && (
          <div className="mt-4">
            <h5 className="font-bold text-sm text-error">Error</h5>
            <p className="text-sm text-error/80 whitespace-pre-wrap break-words font-mono bg-error/10 p-2 rounded mt-1">
              {event.error}
            </p>
          </div>
        )}
          ...
          {isCorsBlocked ? (
            <div className="flex flex-col">
              <div className="text-sm bg-yellow-400/10 text-yellow-500 p-3 rounded-md mb-4">
                <p>
                  <strong>URL blocked (CORS):</strong>{' '}
                  <span className="font-mono break-all">{event?.input_snapshot?.url}</span>
                </p>
                <p className="mt-2">
                  To proceed, please manually copy the content from the page and paste it below.
                </p>
              </div>
              <label htmlFor="pasted-content" className="font-medium text-sm mb-1">
                Pasted Content
              </label>
              <textarea
                id="pasted-content"
              ...
              <button
                ...
                className="w-full bg-primary hover:bg-primary/90 text-secondary font-bold py-2 px-4 rounded-md mt-4 disabled:opacity-50"
              >
            ...
            <button
              onClick={onRetry}
              className="w-full bg-primary hover:bg-primary/90 text-secondary font-bold py-2 px-4 rounded-md"
            >
              Retry Step
            </button>
          )}
...
const OrchestrationTab: React.FC<{
  ...
  apiKeyMissing: boolean;
}> = ({ job, updateJob, traceLog, executePlan, onManualIngest, apiKeyMissing }) => {
  ...
  const phases = useMemo(() => {
    ...
    return Object.entries(phaseMap).map(([name, steps]) => ({
      name: name as ExecutionStep['action'],
      steps,
    }));
  }, [job.plan]);
  ...
  const isExecuting = ['Running', 'Comparing', 'Answering', 'Synthesizing', 'Planning'].includes(
    job.status,
  );
  ...
  const runningStepEvent = runningStep
    ? traceLog
        .slice()
        .reverse()
        .find((e) => e.step_id === runningStep.id)
    : undefined;
  const nextStep = job.plan.steps.find((s) => s.status === 'Pending');

  const selectedStep = job.plan.steps.find((s) => s.id === selectedStepId);
  const selectedEvent = traceLog
    .slice()
    .reverse()
    .find((e) => e.step_id === selectedStepId);
  ...
  const handleRetryStep = (stepId?: string) => {
    ...
    const newSteps = job.plan!.steps.map((s) =>
      s.id === stepId
        ? { ...s, status: 'Pending' as const, result: undefined, duration: undefined }
        : s,
    );
    updateJob({
      ...job,
      plan: { ...job.plan!, steps: newSteps },
      status: 'Running',
      lastError: undefined,
    });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
      <div className="lg:col-span-2 flex flex-col gap-4 overflow-y-auto pr-2">
        ...
        <div className="p-4 bg-light-surface dark:bg-dark-surface rounded-lg">
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              onClick={() => executePlan()}
              disabled={isExecuting || job.status === 'Complete' || apiKeyMissing}
              className="bg-primary hover:bg-primary/90 text-secondary font-bold py-2 px-4 rounded-md disabled:bg-light-text/20 dark:disabled:bg-dark-text/20"
            >
          ...
          <div className="text-sm mt-2 text-light-text/80 dark:text-dark-text/80 h-5 flex justify-between items-center">
              ...
              {runningStep ? (
                <>
                  ...
                  <span className="truncate">
                    {runningStep.action} – {runningStepEvent?.summary || '...'}
                  </span>
                </>
              ) : isExecuting ? (
                <>
            ...
            {nextStep && job.status !== 'Complete' && job.status !== 'Error' && (
              <span className="text-light-text/60 dark:text-dark-text/60">
                Up next: <span className="font-semibold">{nextStep.action}</span>
              </span>
            )}
          </div>
        ...
        <div className="space-y-4">
          {phases.map((phase) => (
            <div key={phase.name} className="bg-light-surface dark:bg-dark-surface rounded-lg">
              <button
                onClick={() => setCollapsedPhases((p) => ({ ...p, [phase.name]: !p[phase.name] }))}
                className="w-full flex justify-between items-center p-3 text-left"
              >
                <h4 className="text-lg font-bold capitalize">
                  {phase.name.replace(/_/g, ' ')} ({phase.steps.length})
                </h4>
                <ChevronDownIcon
                  className={`w-5 h-5 transition-transform ${collapsedPhases[phase.name] ? 'rotate-180' : ''}`}
                />
              </button>
      ...
      <div className="lg:col-span-1 bg-light-surface dark:bg-dark-surface rounded-lg h-full overflow-y-auto">
        <DetailPanel
          step={selectedStep}
          event={selectedEvent}
          onRetry={() => handleRetryStep(selectedStepId || undefined)}
...
const MainContent: React.FC<MainContentProps> = ({
  ...
  if (!job) {
    return (
      <div className="flex-grow flex items-center justify-center text-light-text/60 dark:text-dark-text/60">
        Select a job or create a new one to start.
      </div>
    );
  }
  ...
  return (
    <div className="flex-grow flex flex-col p-6 bg-light-bg dark:bg-dark-bg h-screen overflow-hidden">
          ...
          {activeView === 'scope' && (
            <div className="relative group">
              <button
                ...
                disabled={!isPlanReady || job.status === 'Planning' || apiKeyMissing}
              ...
              {!isPlanReady && (
                <div className="absolute bottom-full right-0 mb-2 w-max max-w-[250px] bg-secondary text-dark-text text-xs rounded-lg py-2 px-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 shadow-lg border border-primary/20">
                  Please provide an objective and at least one key question to enable plan
                  generation.
                </div>
              )}
            </div>
      ...
      <div className="flex-grow overflow-y-auto">
        {activeView === 'scope' && (
          <ScopeTab job={job} updateJob={updateJob} apiKeyMissing={apiKeyMissing} />
        )}
        {activeView === 'orchestration' && (
          <OrchestrationTab
            ...
            updateJob={updateJob}
            traceLog={traceLog}
            executePlan={(resume, stepId) => executePlan(job, resume, stepId)}
            onManualIngest={handleManualIngest}
